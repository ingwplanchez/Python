...
#-----------------------------------------------------------------------------
# Funcion Scan Con parametros
# num_ports : Numero de puertos
# verbose: variable del tipo booleana

def scan(num_ports = 20, verbose=True):

    #-- Lista de los dispositivos serie. Inicialmente vacia
    dispositivos_serie = [] # inicializacion del vector (Vacio)

    if verbose: # si es true
      print "Escanenado %d puertos serie:" % num_ports

    #-- Escanear num_port posibles puertos serie
    # Para todos los elementos de i en un rango de 0 a 19 hacer
    for i in range(num_ports):

      if verbose: # si es verdadero
